...
describe("App runner onboarding", () => {
  ...
  it("opens the runner manager and requires confirmation before launching auth", async () => {
    const launchRunnerAuth = vi.fn(async () => {});
    ...
    useSessionStateMock.mockReturnValue(
      createBaseState({
        runnerStatus: createRunnerStatus({
          ...
          authCommand: "codex",
          authInstructions: 'OpenPlot will open Terminal and run "codex" for you.',
        }),
        launchRunnerAuth,
      }),
    );
    ...
    const openButton = Array.from(document.querySelectorAll("button")).find(
      (button) => button.textContent === "Open runners",
    );
    if (!openButton) {
      throw new Error("Expected mocked toolbar button");
    }
    ...
    const authenticateButton = Array.from(document.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Authenticate"),
    );
    if (!authenticateButton) {
      throw new Error("Expected authenticate button in runner manager");
    }
    ...
    expect(document.body.textContent).toContain("Authenticate Codex");
    expect(launchRunnerAuth).not.toHaveBeenCalled();

    const confirmButton = Array.from(document.querySelectorAll("button")).find(
      (button) => button.textContent === "Open Terminal",
    );
    if (!confirmButton) {
      throw new Error("Expected auth confirmation button");
    }
    ...
    expect(launchRunnerAuth).toHaveBeenCalledWith("codex");
  });
});
